#!/usr/bin/env python3
# theme_apply.py <Palette>                  -> GLOBAL (persiste + reconstruit l'accueil)
#                --swayonly                  -> boot : re-applique la palette courante a sway
#                --activity <nom> <Palette>  -> override pour une activite (persiste + applique)
import sys, os, subprocess, time, json
import os; sys.path.insert(0, os.path.dirname(os.path.realpath(__file__)))
import focus_theme
CONF = os.path.expanduser("~/.config/focus")

def sw(c): subprocess.run(["swaymsg", "-q", c])
def focused():
    try:
        for w in json.loads(subprocess.check_output(["swaymsg", "-t", "get_workspaces"])):
            if w.get("focused"): return w.get("name")
    except Exception:
        pass
    return None

a = sys.argv[1:]

if a and a[0] == "--swayonly":
    focus_theme.apply_sway()
    sys.exit(0)

if a and a[0] == "--activity":
    name = a[1]
    pal = a[2] if len(a) > 2 else focus_theme.current()
    if pal not in focus_theme.PALETTES: pal = "Lavande"
    os.makedirs(CONF + "/themes", exist_ok=True)
    open(CONF + "/themes/" + focus_theme.slug(name), "w").write(pal)
    focus_theme.apply_sway(pal); focus_theme.write_waybar(pal); focus_theme.reload_waybar()
    print("activite %s -> %s" % (name, pal))
    sys.exit(0)

pal = a[0] if a else focus_theme.current()
if pal not in focus_theme.PALETTES: pal = "Lavande"
os.makedirs(CONF, exist_ok=True)
open(CONF + "/theme", "w").write(pal)
focus_theme.apply_sway(pal); focus_theme.write_waybar(pal); focus_theme.reload_waybar()
cur = focused()
sw("workspace Accueil")
sw("[app_id=focus-home] kill"); sw("[app_id=focus-applet] kill"); time.sleep(1)
subprocess.run(["python3", os.path.join(os.path.dirname(os.path.realpath(__file__)), "activity.py"), "home"]); time.sleep(1)
if cur and cur != "Accueil":
    sw("workspace " + cur)
print("global -> %s" % pal)
